...
async fn main(_spawner: Spawner) {
    ...
    let mut spi = Spi::new_with_dma(spi0, sck, mosi, miso, p.DMA_CH0, p.DMA_CH1, spi::Config::default());
    let mut got = [0u8; 64];
    with_timeout(Duration::from_millis(200), spi.transfer(&mut got, &pattern))
        .await
    ...
    let (ch0, ch1) = unsafe { (peripherals::DMA_CH0::steal(), peripherals::DMA_CH1::steal()) };
    let mut config = spi::Config::default();
    config.frequency = 8_000_000;
    let mut spi = Spi::new_with_dma(spi0, sck, mosi, miso, ch0, ch1, config);
