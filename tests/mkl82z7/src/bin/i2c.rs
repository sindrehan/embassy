...
async fn main(_spawner: Spawner) {
    ...
    // An expired deadline must fail even with a coarse time driver.
    let mut timeout_config = config.clone();
    timeout_config.timeout = Duration::from_ticks(0);
    ...
    // Cancel an armed transfer before its interrupt can complete, then recover the bus.
    poll_fn(|cx| {
        cortex_m::interrupt::free(|_| {
            let mut transfer = pin!(i2c.write(FXOS8700, &[REG_WHO_AM_I]));
    ...
    defmt::assert_eq!(i2c.write(0x7E, &[0]).await, Err(Error::AddressNack));
